...
방출한 뒤 턴을 끝낸다. emit은 presentation/api.py의 스레드→큐 브릿지가 소비한다.

모듈 경계: 팩트 적립은 facts.facade, plan 세대 생성은 plans.facade로 —
타 모듈 테이블을 직접 다루지 않는다 (가이드 §3 Facade).
"""
...
class InterviewAgent:
    ...
    def _turn_context(self) -> str:
        ...
        if src:
            parts.append("### 소스 문서\n" + src)
        facts = list_active_facts(self.db, self.sess.project_id)
        if facts:
    ...
    def _write_plan(self, args: dict, events: _EventSink) -> str:
        ...
        p = create_generation(self.db, self.sess.project_id, markdown=md, docs=plan.docs,
                              parsed_ok=True, origin=PlanOrigin.INTERVIEW)
        ...
        events.add(Event(name="plan_draft", kind=MessageKind.PLAN_DRAFT,
                         payload={"plan_id": p.id, "version_no": p.version_no,
                                  "docs": plan.docs, "markdown": md}))
        self._append(MessageRole.EVENT, MessageKind.PLAN_DRAFT,
                     payload={"plan_id": p.id, "version_no": p.version_no})
        events.add(state_event(self.sess.phase.value, self.sess.round_no, self.sess.checklist))
        return f"OK: plan v{p.version_no}이 저장됐다. 사용자 승인을 기다린다."
    ...
    def commit_facts(self, edits: list[dict] | None = None) -> list[dict]:
        ...
        append_facts(self.db, self.sess.project_id, self.sess.id, pending)
        lines = []
        for f in pending:
