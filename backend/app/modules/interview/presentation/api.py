...
직접 반환한다(D6). LLM 턴은 application/agent.py 상태머신이 워커 스레드에서 실행하고,
...
from app.modules.projects.facade import project_slug, require_project
from app.modules.sources.facade import project_sources_dir
from app.shared.db import get_db, make_session_factory
from app.shared.errors import http_404, http_409
from app.shared.workspace import workspace_path
...
def get_session(session_id: int, db: Session = Depends(get_db)):
    return SessionOut.model_validate(require_session(db, session_id))
...
def _make_agent(db, sess, settings, llm_overrides):
    ...
    ws = workspace_path(settings, project_slug(db, sess.project_id))
    sources_dirs = [settings.global_sources_dir, project_sources_dir(db, sess.project_id)]
