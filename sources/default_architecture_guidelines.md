...
> - 본 문서의 구체적 예시(모듈명, PG 연동, 기본 스택 등)는 예시일 뿐이며, 프로젝트 도메인에 맞게 치환한다.
> - **버전:** v1.0 — 2026-09-23 (적용 전제·기본 스택 명시, §5 데이터·API 규약·§6 테스트·배포 추가)

본 문서는 프로젝트 계획 수립(인터뷰·개발설계서 작성)과 구현 단계(AI 코딩 어시스턴트, 팀원)에서 참조할 아키텍처 규칙, 디렉토리 구조, 디자인 패턴, 데이터·API 규약, 테스트·배포 전략의 기본 권장 컨벤션을 제안한다.

---

## 0. 적용 전제와 기본 스택 (예시)

본 문서는 **웹 애플리케이션(백엔드 API 서버 + React SPA)** 프로젝트를 전제로 한다. 그 외
프로젝트 유형은 항목을 치환해 적용한다. 기본 스택은 아래와 같다 — 팀 표준 스택이 있으면
그것을 우선한다.

- **백엔드:** Python 3.12 + FastAPI + SQLAlchemy 2.x — 비동기 지원·타입 힌트 친화·경량
- **프론트엔드:** React + TypeScript + Vite — 컴포넌트 생태계·빠른 빌드
...
2. **엄격한 경계:** 모듈 간 직접 참조는 명시적인 공개 인터페이스(모듈 퍼사드 — `user_facade.py`)로만 허용한다. 다른 모듈의 DB 테이블이나 내부 Internal Repository에 직접 접근하는 것은 금지한다.
3. **이벤트 기반 결합 (선택):** 모듈 간 강한 결합을 해제할 필요가 있을 때 도메인 이벤트를 활용한다. 구현은 인프로세스 동기 디스패처로 시작하고, outbox·메시지 큐 같은 비동기 구조는 트래픽·일관성 요건이 확인될 때 확정한다.
...
│   │   └── user_facade.py       # 타 모듈에 제공하는 모듈 퍼사드(공개 인터페이스)
│   ├── payment/                 # [결제 도메인 모듈]
...
* **상태 분리:** 서버 상태(API 응답 캐싱·동기화)는 ViewModel 훅 안에서 쿼리 라이브러리(예: TanStack Query)로 취급하고, 전역 스토어(shared)에는 클라이언트 상태만 둔다.
...
### 3.1 Facade 패턴 (외부 시스템 연동)
* **적용 대상:** PG 연동(예시: 토스, 카카오페이), 외부 알림 서비스(SMS, Email) 등 복잡한 외부 SDK. 모듈 간 통신에는 Facade가 아니라 §1.1의 모듈 퍼사드를 사용한다.
* **원칙:** 복잡한 외부 SDK와 직접 상호작용하지 않고 단일 `Facade` 클래스를 거치도록 구현한다.

```python
# 예시: 외부 결제 Facade
class PaymentFacade:
    def process_payment(self, order_id: str, amount: int) -> PaymentResult:
        """외부 SDK 호출과 복잡한 세부 절차를 은닉한다."""
```
...
## 5. 데이터 모델·API 규약 (기본 제안)

1. **API 스타일:** REST + JSON, 리소스 중심 경로를 사용하고 오류 응답은 프로젝트 단일 스키마(상태 코드 + 오류 코드 + 메시지)로 통일한다. API 문서는 OpenAPI 자동 생성으로 유지하고, 스키마 고정·코드 생성이 필요하면 계약 우선 OpenAPI로 확정한다.
2. **데이터 소유:** 테이블은 소속 모듈이 소유한다(§1.1 경계와 일관). 타 모듈 테이블은 모듈 퍼사드·API로만 접근한다.
3. **공통 컬럼 규약:** 모든 테이블에 생성·수정 시각(created_at, updated_at)을 두고, 삭제 정책(물리 삭제 vs 소프트 삭제)은 프로젝트 단위로 하나만 채택한다.
4. **스키마 관리:** 마이그레이션 도구(예시: Alembic)로 스키마 변경 이력을 관리한다 — 수동 DDL은 금지한다.

---

## 6. 테스트·배포 전략 (기본 제안)

1. **테스트 범위:** 단위 + 통합 테스트 위주로 CI에서 전체 실행한다. E2E는 핵심 시나리오 한정으로 유지한다.
2. **외부 의존 격리:** 외부 API 등 외부 의존은 fake·스텁을 주입해 테스트한다 — 실제 외부 호출을 테스트에 두지 않는다.
3. **배포:** 컨테이너 이미지 기반 단일 배포(§1 모놀리식과 일관)를 기본으로 한다. 환경별 설정은 코드에 두지 않고 환경변수·설정 파일로 주입한다.
4. **롤백:** 롤백 기준(예시: 핵심 시나리오 실패, 오류율 임계 초과)과 절차(이전 이미지 태그 재배포)를 배포 전에 명시한다.